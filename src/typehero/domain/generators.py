...
Stage shape (a positive-int `count`/`length`, a non-empty `keys`/`file`) is
validated by the content loader, so the generators trust their typed inputs and
only guard the failures that depend on the data itself (an empty word pool, an
unknown corpus file).
"""
...
def _render_wordlist(
    stage: dict[str, object], *, wordlist: Sequence[str], rng: random.Random
) -> str:
    count = cast("int", stage["count"])
    keys = set(cast("str", stage["keys"]))
...
def _render_corpus(
    ...
    file = cast("str", stage["file"])
    length = cast("int", stage["length"])
